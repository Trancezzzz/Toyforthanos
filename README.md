...
- **Logo (v2.6.1)** — icon is now self-hosted in this repo; nyaa.si sends no CORS headers, and Seanime loads extension icons with `crossOrigin="anonymous"`, so the logo was silently blocked. raw.githubusercontent.com serves `Access-Control-Allow-Origin: *`, so the Nyaa logo renders
- **Title OR group (v2.7)** — fetch parity with the built-in provider. The exact-phrase group used to AND the episode/resolution suffix onto every title variant *inside* the OR; live-verified that nyaa's parser returns **0 rows** once 3+ variants carry a suffix group (`("a"(05)|"b"(05)|"c"(05))` → 0, but `("a"|"b"|"c")(05)` → hits). The group now carries ALL titles (incl. CJK variants — harmless inside an OR, adds coverage) with episode + resolution ANDed *after* it, exactly like Seanime's built-in nyaa provider (which also carries `e05v`/`05v` version markers — now included too). The 3-title cap is gone; movie mode gets the group as well
